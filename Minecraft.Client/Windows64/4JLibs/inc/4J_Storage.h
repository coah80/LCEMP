...
public:
	...
	void						SetDefaultImages(PBYTE pbOptionsImage,DWORD dwOptionsImageBytes,PBYTE pbSaveImage,DWORD dwSaveImageBytes,PBYTE pbSaveThumbnail,DWORD dwSaveThumbnailBytes);	// Sets default save image and thumbnail
	void						GetDefaultSaveImage(PBYTE *ppbSaveImage,DWORD *pdwSaveImageBytes);																					// Get the default save image (as set by SetDefaultImages)
	...
	void						ContinueIncompleteOperation();																														// Continue a previously incomplete save operation
	C4JStorage::ESaveGameState	GetSaveState();																																		// Returns the current state of the save system
	...
	C4JStorage::ESaveGameState		RenameSaveData(int iRenameIndex,uint16_t*pui16NewName,int( *Func)(LPVOID lpParam,const bool), LPVOID lpParam);										// Rename savedata with index from last established ReturnSavesInfo.
