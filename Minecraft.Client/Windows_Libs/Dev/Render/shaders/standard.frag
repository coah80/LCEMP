...
// PS_Standard — Standard textured fragment shader with alpha test and fog
...
void main() {
    // Use explicit LOD 0 when U > 1.0 (atlas/wrapped), otherwise auto-mip
    vec4 texColor;
    if (vTex.x > 1.0) {
        texColor = textureLod(diffuse_texture, vTex.xy, 0.0);
    } else {
        texColor = texture(diffuse_texture, vTex.xy);
    }
